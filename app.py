...
from utils import write_results, get_dates, to_canvas, to_lonlat, transform_coords
...
class App:
    def __init__(self, master):
        ...
        self.canvas.get_tk_widget().grid(row=2, column=4, columnspan=6,
                                         rowspan=7)

        self.toolbar_frame = tk.Frame(self.master)
        self.toolbar_frame.grid(row=9, column=4, columnspan=6)
        ...
        self.start_entry.grid(row=2, column=2, columnspan=2)
        ...
        self.lon_entry.grid(row=3, column=2)

        self.lon_entry = tk.Entry(master, textvariable=self.lat)
        self.lon_entry.config(width=5)
        self.lon_entry.grid(row=3, column=3)

        self.k = tk.Scale(master, label='K* (persons/100 km2)', from_=20, to=100, resolution=20, orient=tk.HORIZONTAL, length=200)
        self.k.grid(row=4, column=1, columnspan=3)

        self.fiss = tk.Scale(master, label='Fission threshold', from_=50, to=300, resolution=50,
                             orient=tk.HORIZONTAL, length=200)
        self.fiss.grid(row=5, column=1, columnspan=3)

        self.catch = tk.Scale(master, label='Catchment (km)', from_=10, to=30, resolution=10,
                              orient=tk.HORIZONTAL, length=200)
        self.catch.grid(row=6, column=1, columnspan=3)

        self.leap = tk.Scale(master, label='Leap distance (km)', from_=0, to=300, resolution=50,
                             orient=tk.HORIZONTAL, length=200)
        self.leap.grid(row=7, column=1, columnspan=3)

        self.perm = tk.Scale(master, label='Permanence (years)', from_=10, to=30, resolution=10,
                             orient=tk.HORIZONTAL, length=200)
        self.perm.grid(row=8, column=1, columnspan=3)

        # Controls
        self.setup_button = tk.Button(master, text='Setup', width=10,
                                      command=self.setup_model)
        self.setup_button.grid(row=1, column=4)
        ...
        self.iter_thousand_button = tk.Button(master, text='▶ 1000 yrs', width=10,
                                              command=self.iter_thousand)
        self.iter_thousand_button.grid(row=1, column=7)

        self.write_button = tk.Button(master, text='Write', width=10,
                                      command=self.write_model)
        self.write_button.grid(row=1, column=8)

        self.eval_button = tk.Button(master, text='Evaluate', width=10,
                                     command=self.eval_model)
        self.eval_button.grid(row=1, column=9)
    ...
    def plot_model(self):
        ...
        self.ax.text(580, 50, 'yr BP', color='white', horizontalalignment='right')
