...
class Genome: 
    ...
    def initialize(self): 
        ...
        self.genes = [Gen_01(), Gen_02(), Gen_04(), Gen_05()]
        for i in self.genes:
            i.initialize()
    ...
    def genes_values(self): 
        values = [i.value for i in self.genes]
        values.insert(2, 2)                                                         # catchment
        return values
    def Print(self): 
        print('\nFitness: %s\n' % self.fitness)
    ...
    def evolve(self):
        ...
        for it in range(self.max_it):
            ...
            self.population = mutated + cp.deepcopy([parents[i] for i in
                                                    range(self.n_elit)]) 
            self.fitAve.append(np.mean(fitness[np.isfinite(fitness)]))                                        # delete
            self.fitMax.append(np.max(fitness[np.isfinite(fitness)]))                                         # delete
            bar.next()                                                                                   # delete
            if it % 10 == 0:                                                                          # set write
                FILE = open('./results/GA.pyData', 'wb')
                print('\n it: ' + str(it) + ' writing ...\n')
            ...
            for i in parents[0:5]:                                                                 # experimental
                i.Print()
        with open('./results/GA.csv', 'w') as result_file:
            result_file.write('it,avg,max\n')
            n = 1
            for avgf, maxf in zip(self.fitAve, self.fitMax):
                result_file.write('{},{},{}\n'.format(n, avgf, maxf))
                n += 1
        bar.finish()                                                                                     # delete
        plt.plot(self.fitAve)                                                                                 # delete
        plt.plot(self.fitMax)
        plt.show()                                                                                       # delete
           

def main():
    my_ga = GA(100, 40, 5, 0.8, 0.2, 21)                                                              # parameters
    my_ga.initialize()
    my_ga.evolve()
